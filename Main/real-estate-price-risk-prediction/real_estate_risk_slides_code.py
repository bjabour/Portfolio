...
PDF = ROOT / "real_estate_risk_slides.pdf"
TRAIN = ROOT / "real_estate_risk_train.csv"
RESULTS = ROOT / "real_estate_risk_results.json"
...
def save(pdf, fig, number, bottom=.045, top=.915):
    ...
    pdf.savefig(fig)
    fig.savefig(ROOT / f"real_estate_risk_slide_preview_{number}.png", dpi=150)
    plt.close(fig)
...
def slide2(pdf, results):
    ...
    text_panel(fig.add_subplot(gs[0, 0]), "Libraries", [
        "ג€¢ numpy / pandas: data, arrays, JSON",
        "ג€¢ sklearn: permitted tree, RF classifier,",
        "  sigmoid calibration and validation",
        "",
        "ג€¢ imbalanced-learn: fold-local oversampling",
        "ג€¢ XGBoost 3.2: Tasks 3 and 5",
        "",
        "ג€¢ matplotlib / seaborn: plots and slides",
        "ג€¢ Tasks 1-2 use self-written CART/RF",
    ], 9.6, body_y=.86, linespacing=1.28)
    text_panel(fig.add_subplot(gs[0, 1]), "Assumptions & Notes", [
        "ג€¢ Train and test share the same mechanism.",
        "ג€¢ Rows are independent properties.",
        "",
        "ג€¢ Original year and area values are retained.",
        "ג€¢ Trees model nonlinearities and interactions.",
        "",
        "ג€¢ Outer validation is untouched by tuning,",
        "  resampling, early stopping or calibration.",
        "ג€¢ Importance is predictive, not causal.",
    ], 9.6, body_y=.86, linespacing=1.28)
    ...
    numeric = pd.DataFrame([
        ["1 CART", f"{min(results['tree_cv_mse_curve']):.2f}", "equiv. max ־”", f"{max(abs(np.array(results['tree_pred_test'])-np.array(results['tree_lib_pred_test']))):.1e}"],
...
def main():
    if not RESULTS.exists():
        raise FileNotFoundError("Run real_estate_risk_code.py before generating slides.")
