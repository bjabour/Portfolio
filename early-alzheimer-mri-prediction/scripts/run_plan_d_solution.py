...
SPLIT_DIRS = {
    "train": Path("data") / "splits" / "train" / "train",
    "val": Path("data") / "splits" / "val" / "val",
    "test": Path("data") / "splits" / "test" / "test",
}
...
- Script: `scripts/run_plan_d_solution.py`
- Seed: `{seed}`
...
def run_deterministic_stage(project_root: Path, seed: int) -> Path:
    output_dir = project_root / "results" / "experiments" / "plan_d"
