...
def fit_current_lasso(x: np.ndarray, y: np.ndarray) -> tuple[LassoCV, np.ndarray, np.ndarray]:
    ### Reproduces the final global-scaling plus LassoCV workflow
...
def run_replication(
    ...
    # Current lasso reproduces the final preprocessing and default CV behavior.
...
def write_summary(
    ...
    for label, key in [
        ...
        ("Ridge, final single 5-fold CV style", "ridge_single"),
        ...
        ("Lasso, final LassoCV style", "lasso_current"),
